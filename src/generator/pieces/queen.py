...
class Queen(Piece):
    ...
    def generate_capture_moves(self, source_square):
        ...
        attacks = self.get_queen_attacks(source_square) & offset
    ...
    def get_queen_attacks(self, source_square):
        occupancy = self.occupancies[SIDES["all"]]
        attack_tables = (self.bishop_attack_table, self.rook_attack_table)
        attack_masks = (self.bishop_attack_masks, self.rook_attack_masks)

        return Attack.get_queen_attack_masks(source_square, occupancy, attack_tables, attack_masks)
